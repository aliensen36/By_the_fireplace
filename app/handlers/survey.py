...
from app.fsm_states import SurveyState
...
async def survey_start(message: Message, state: FSMContext):
    ...
    await state.set_state(SurveyState.age_group)


# Обработчик выбора возраста (вопрос 1)
@survey_router.callback_query(SurveyState.age_group,
...
async def survey_age_group(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.residence)


# Обработчик выбора места жительства (вопрос 2)
@survey_router.callback_query(SurveyState.residence,
...
async def survey_residence(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.company)


# Обработчик выбора компании (вопрос 3)
@survey_router.callback_query(SurveyState.company,
...
async def survey_company(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.reason)


# Обработчик выбора причины (вопрос 4)
@survey_router.callback_query(SurveyState.reason,
...
async def survey_reason(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.advertising_sources)


# Обработчик текстового ответа на вопрос 4
@survey_router.message(SurveyState.reason)
...
@survey_router.callback_query(SurveyState.advertising_sources,
...
async def survey_advertising_sources(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.visit_frequency)


# Обработчик выбора частоты посещения (вопрос 6)
@survey_router.callback_query(SurveyState.visit_frequency,
...
@survey_router.callback_query(SurveyState.purpose,
...
async def survey_purpose(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.food_preferences)


# Обработчик выбора предпочтений (вопрос 8)
@survey_router.callback_query(SurveyState.food_preferences,
...
async def survey_food_preferences(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.suggestions)


# Обработчик текстового ответа на вопрос 8
@survey_router.message(SurveyState.food_preferences)
...
@survey_router.callback_query(SurveyState.suggestions)
async def survey_suggestions(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.atmosphere)


# Обработчик текстового ответа на вопрос 9
@survey_router.message(SurveyState.suggestions)
...
@survey_router.callback_query(SurveyState.atmosphere,
...
async def survey_atmosphere(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.service_rating)


# Обработчик оценки обслуживания (вопрос 11)
@survey_router.callback_query(SurveyState.service_rating,
...
async def survey_service_rating(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.improvements)


# Обработчик текстового ответа на вопрос 11
@survey_router.message(SurveyState.service_rating)
...
@survey_router.callback_query(SurveyState.improvements,
...
async def survey_improvements(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.obstacles)


# Обработчик текстового ответа на вопрос 12
@survey_router.message(SurveyState.improvements)
...
@survey_router.callback_query(SurveyState.obstacles,
...
async def survey_obstacles(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.restaurants)


# Обработчик для типов ресторанов (вопрос 14)
@survey_router.callback_query(SurveyState.restaurants,
                              F.data.in_(['chain_restaurants', 'fine_dining',
                                          'cafes', 'bars']))
async def survey_restaurants(callback: CallbackQuery, state: FSMContext):
    await state.update_data(restaurants=callback.data)
    await callback.answer()
    await callback.message.edit_text("15. Как вы предпочитаете "
                                     "получать новости ресторана?",
                                     reply_markup=inline_kb.kb_news)
    await state.set_state(SurveyState.news)


# Обработчик способа получения новостей (вопрос 15)
@survey_router.callback_query(SurveyState.news,
...
async def survey_news(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("16. Что бы вы пожелали нашему ресторану? "
                                     "Напишите текстом.",
                                     reply_markup=inline_kb.kb_skip)
    await state.set_state(SurveyState.wishes)


# Обработчик текстового ответа на вопрос 15
@survey_router.message(SurveyState.news)
...
@survey_router.callback_query(SurveyState.wishes)
async def survey_wishes(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurveyState.recommendation)


# Обработчик текстового ответа на вопрос 16
@survey_router.message(SurveyState.wishes)
...
@survey_router.callback_query(SurveyState.recommendation)
async def survey_recommendation(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("18. И последний вопрос. Почему вы "
                                     ...
                                     reply_markup=inline_kb.kb_skip)
    await state.set_state(SurveyState.explanation)


# Обработчик для пояснения (вопрос 18)
@survey_router.callback_query(SurveyState.explanation)
async def survey_explanation(callback: CallbackQuery, state: FSMContext,
                             session: AsyncSession):
    await state.update_data(explanation=callback.data)
    await callback.answer()

    data = await state.get_data()
    await orm_survey(session, tg_id=callback.from_user.id, data=data)
...
@survey_router.message(SurveyState.explanation)
async def survey_explanation_text(message: Message, state: FSMContext,
    ...
    data = await state.get_data()
    await orm_survey(session, tg_id=message.from_user.id, data=data)
